...
const skills = {
  ...
  olpingtao: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target.chooseToGive(player, `${get.translation(player)}对你发动了【平讨】`, `交给其一张牌并令其本回合使用的下一张【杀】可额外选择一个目标${bool ? "；或点击“取消”令其视为对你使用一张【杀】" : ""}`, "he").set("ai", (card2) => {
    ...
    subSkill: {
      sha: {
        ...
        async cost(event, trigger, player) {
          ...
          event.result = await player.chooseTarget(`平讨：为此杀额外指定至多${get.cnNumber(num, true)}个目标`, [1, num], (card, player2, target) => {
            return get.event().targetx.includes(target);
          }).set("targetx", targets).set("ai", (target) => {
