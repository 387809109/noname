...
const skills = {
  ...
  sbzhenwei: {
    ...
    chooseToDiscard(player, source, target, eventId) {
      ...
      next.set("ai", (card) => {
        ...
        if (player2 === sourcex || player2 === targetx) {
          return ui.selected.cards.length < 3 ? 6 - get.value(card) : 0;
        }
        return get.attitude(player2, sourcex) > 0 ? 5.5 - get.value(card) : 0;
      });
      ...
      if (num > 0) {
        const result = await player.chooseButton({
          ...
          switch (button.link) {
            ...
            case "hujia":
              return player2.hujia < 5 ? get.recoverEffect(player2, player2, player2) : 0;
          }
        }).forResult();
      ...
      gain: {
        ...
        async content(event, trigger, player) {
          ...
          while (true) {
            ...
            const cards2 = trigger.getg(target).filter((card) => get.type(card) === "trick" && lib.filter.canBeGained(card, target, player));
            const num = lib.skill.sbqicai.getLimit - target.countMark(skill);
            let result = { bool: true, links: cards2 };
            if (cards2.length > num) {
              result = await target.chooseButton({
                createDialog: [`奇才：将其中${get.cnNumber(num)}张牌交给${get.translation(player)}`, cards2],
                selectButton: num,
                forced: true,
      ...
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          const result = await player.chooseTarget({
            ...
            selectTarget: [1, 3],
            forced: true,
            filterTarget: (card, player2, target) => player2.canUse("sha", target, false),
            ai: (target) => {
              const player2 = _status.event.player;
              return get.effect(target, { name: "sha" }, player2, player2);
            }
  ...
  sbkurou: {
    ...
    async content(event, trigger, player) {
      ...
      await player.give(cards2, target);
      await player.loseHp(["tao", "jiu"].includes(get.name(cards2[0], target)) ? 2 : 1);
    },
    group: "sbkurou_gain",
    ai: {
      nokeep: true,
...
const dynamicTranslates = {
  ...
  sbzhenliang(player) {
    ...
    let yang = "出牌阶段限一次，你可以弃置X张与“任”颜色相同的牌并对攻击范围内的一名角色造成1点伤害（X为你与其体力值值差且X至少为1）", yin = "你的回合外，一名角色使用或打出牌结算完成后，若此牌与“任”类别相同，则你可以令至多两名角色各摸两张牌";
    if (bool) {
